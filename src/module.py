...
class SegmentEncoding(nn.Module):
    ...
    def forward(self, x, type_input=None): # typ_input : 0, 1 으로 마스킹 되있는 input data, x.size == type_input.size
        bs, seq = x.size()
        if type_input is None:  # type_token_num ==1 (한 종류의 문장만 들어감)
            type_input = torch.zeros((bs, seq), dtype=torch.long, device=x.device)
